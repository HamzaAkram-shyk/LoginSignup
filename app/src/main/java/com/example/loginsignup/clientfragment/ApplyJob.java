...
public class ApplyJob extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mainView = inflater.inflate(R.layout.fragment_apply_job, container, false);
    ...
    private void fetchTask() {
        ...
        tasDb.collection("UserJob").whereEqualTo("workerId", user.getUid())
                .get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {

                for (QueryDocumentSnapshot documentSnapshot : task.getResult()) {
                    jobList.add(documentSnapshot.toObject(_UserJobs.class));
                }
                loading.setVisibility(View.GONE);
                adapter = new MyJobsAdapter<_UserJobs>(getContext(), jobList);
